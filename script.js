...
function scoresToDOM(){
  ...
  updateBestScore();
  // reset item container, scroll to top
...
function checkTime(){
  if(playerGuessArray.length == questionAmount){
    ...
    clearInterval(timer);
    // check for wrong guess then add penalty time
    equationsArray.forEach((equation, index) => {
      if(equation.evaluated === playerGuessArray[index]){
        // correct guess, no penalty

      }else{
        // incorrect guess, add a penalty
        penaltyTime += 0.5;
      }
    });
...
// Scroll and store user selection in the playerguess array
...
// On load, set and check local storage
getSavedBestScores();